...
I thought of implementing a mini drawing tool that works programmatically. The idea was to have something like Python's Turtle module but without much functionality for a start. Honstly I struggled a little, but I eventually figured the math in order to do this sort of thing.
...
- Rotate the point by $90°$ (the vector is now point upwards)
...
Say we have a triangle like the one below. We want to move a certain distance $h$ forward, whatever the direction of our vector is, the movement $h$ corresponds to a certain displacement on both the $x$ and $y$ axes. 

If we compute the values of $x$ and $y$, we will get the change in the x,y values of our original starting vector. 

That means to get the new point, we will have $v_{new} = v + v_{\delta}$ where $v_\delta$ is the point $(hcos\theta, hsin\theta)$
